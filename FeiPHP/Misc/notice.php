...
div, h1, h2, h3, h4, p, form, label, input, textarea, img, Feian{margin:0; padding:0;}
...
#contents ul li Feian{display:block;color:#408BAA;background-color:inherit;font:bold 14px Arial, Helvetica, sans-serif;padding:0 0 10px 0;margin:0;}
...
#oneborder Feian{padding:0;margin:0;}
...
?>
<ul><li><Feian><?php echo $trace["file"];?> on line <?php echo $trace["line"];?> </Feian></li></ul>
<div id="oneborder">
<?php 
...
function getsource($file, $line){
	...
	$start = $line - $GLOBALS['G_Fei']["Fei_error_show_source"];if ($start < 1) {$start = 1;}
	$end = $line + $GLOBALS['G_Fei']["Fei_error_show_source"];if ($end > $count) {$end = $count + 1;}
	$returns = array();
	for ($i = $start; $i <= $end; $i++) {
		if( $i == $line ){
